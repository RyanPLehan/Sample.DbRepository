...
namespace Sample.DbRepository.Domain.Asserts
{
    /// <summary>
    ...
    {
        public static void AssertNotLessThan<T>(T value, T minimumValue) where T : IComparable
        {
            if (value.CompareTo(minimumValue) == -1)
            {
                var msg = String.Format("The argument '{0}' is less than '{1}'", nameof(value), nameof(minimumValue));
        ...
        public static void AssertNotDelegate<T>(T function, string name) where T : System.Delegate
        {
            if (function == null || function == default)
                throw new ArgumentException("Delegate Function or Lambda expression must be supplied", name);
        }
