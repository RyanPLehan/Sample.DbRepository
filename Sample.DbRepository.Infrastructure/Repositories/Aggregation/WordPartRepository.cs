...
{
    internal class WordPartRepository : IWordPartRepository
    {
        private const string CACHE_KEY_PREFIX = "WordPartAggregation";
        ...
        public WordPartRepository(IMemoryCache cache,
        ...
        public async Task<IDictionary<char, int>> GetCountByCharacter()
        {
            ...
            if (!_cache.TryGetValue<IDictionary<char, int>>(cacheKey, out statistic))
            {
                ...
                {
                    var query = from w in context.WordParts
